...
class Entidade(db.Model):
    ...
    dataUltimaModificacao = db.Column(db.DateTime, default=db.func.current_timestamp(),
...
class Perfil(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    nome = db.Column(db.String(200), nullable=False, unique=True, info={'label': 'Nome'})
    permissoes = db.relationship('Permissao', secondary=perfis_permissoes, backref=db.backref('perfis', lazy='dynamic'))


class Permissao(db.Model):
    __tablename__ = 'permissoes'
    id = db.Column(db.Integer, primary_key=True)
    url = db.Column(db.String(500), nullable=False, unique=True, info={'label': 'URL'})
...
class Pessoa(db.Model):
    ...
    tipoServidor = db.Column(db.String(2),
                             info={'label': 'Tipo Servidor', 'choices': [(c.name, c.value) for c in TipoServidor]})
    situacaoServidor = db.Column(db.String(1), info={'label': 'Situação do Servidor',
                                                     'choices': [(c.name, c.value) for c in SituacaoServidor]})
    ...
    estadoCivil = db.Column(db.String(2),
                            info={'label': 'Estado Civil', 'choices': [(c.name, c.value) for c in EstadoCivil]})
    ...
    tipoSanguineo = db.Column(db.String(3),
                              info={'label': 'Tipo Sanguíneo', 'choices': [(c.name, c.value) for c in TipoSanguineo]})
    ...
    cpf = db.Column(db.String(200), info={'label': 'CPF'})
    ...
    pagamento_TipoConta = db.Column(db.String(2), info={'label': 'Tipo Conta',
                                                        'choices': [(c.name, c.value) for c in TipoContaBancaria]})
    ...
    jornada = db.Column(db.String(2),
                        info={'label': 'Jornada', 'choices': [(c.name, c.value) for c in JornadaTrabalho]})
...
class CargoFuncaoGratificada(db.Model):
    ...
    classe = db.Column(db.String(2),
                       info={'label': 'Classe', 'choices': [(c.name, c.value) for c in ClasseCargoFuncao]})
    nivel = db.Column(db.String(5), info={'label': 'Nível'})
